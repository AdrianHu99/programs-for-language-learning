...
public class SList {
  ...
  public void squish() {
    ...
    SListNode h1 = head;
    SListNode ans = new SListNode(new Integer(0), h1);
    SListNode h2 = head;
    while (h2.next != null) {
        if (!(h2.item.equals(h2.next.item))) {
            h1.next = h2.next;
            h1 = h1.next;
        }
        h2 = h2.next;
  ...
  public void twin() {
    // Fill in your solution here.  (Ours is seven lines long.)
      /*SListNode h = head;
      SListNode ans = head;
      SListNode h2 = new SListNode(new Integer(0), ans);
      while (h.next != null) {
          ans.next = new SListNode(ans.item);
          ans = ans.next;
          ans.next = h.next;
          ans = ans.next;
          h = h.next;    
      ...
      SListNode h = new SListNode(new Integer(0), head);
      while (head.next != null) {
          SListNode a = new SListNode(head.item);
          a.next = head.next;
          head.next = a;
          head = a.next;
      }
      head.next = new SListNode(head.item);
      head = h.next;
  }
